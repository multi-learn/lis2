...
class ClusterCube(Configurable):
    ...
    config_schema = {
        ...
        "modes": Schema(
            list[Literal["3d_skeletons", "2d_skeletons", "all_3d_skeletons"]],
            default=[],
        ),
    }
    ...
    def __init__(self):
        ...
        if self.subclustering_method:
            self.subclustering_method = BaseSubClustering.from_config(
        ...
        for i, (x, y) in enumerate(points):
            ...
            for idx, s in enumerate(smoothed_signal):
                if (
                    ...
                    and abs(
                        convert_to_kms(
                            self.data3D_reprojected_header["CRPIX3"],
                            self.data3D_reprojected_header["CDELT3"],
                            self.data3D_reprojected_header["CRVAL3"],
                            self.data3D_reprojected_header["NAXIS3"],
                            min(sub_pic),
                        )
                        - convert_to_kms(
                            self.data3D_reprojected_header["CRPIX3"],
                    ...
                    if len(sub_pic) >= self.min_len_souspic:
                        multiplied_points_y_x_z_v_s.extend(
                            [
                                (
                                    y,
                                    x,
    ...
    def clustering_cube(self):
                        ...
                        for label in np.unique(labels):
                            if label != -1:
                                cluster_data = np.zeros(
                                    (self.data3D_reprojected.shape[1:])
                                )
        ...
        fits.writeto(
            filename,
            all_skeletons_all_clusters_data,
            self.data3D_reprojected_header,
            overwrite=True,
        )
