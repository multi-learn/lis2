...
def reproject_2Dspines23Ddata(path_3Ddata, path_3Ddata_reprojected, path_spine):
    ...
    data_2d_cropped = (
        data_spine[0].data[min_pix_y:max_pix_y, max_pix_x:min_pix_x] > 0
    ).astype(np.int8)

    wcs_target_slice = data_s2_wcs.deepcopy()
    wcs_target_slice.wcs.crpix = [
        data_s2_wcs.wcs.crpix[0] - max_pix_x,
        data_s2_wcs.wcs.crpix[1] - min_pix_y,
    ]

    # Reproject 3d data into 2D mask resolution
    target_shape_2d = data_2d_cropped.shape
    reprojected_data_3d = np.empty(
        (data_3D[0].data.shape[0], target_shape_2d[0], target_shape_2d[1])
    )

    for i in range(data_3D[0].data.shape[0]):
        slice = data_3D[0].data[i, :, :]
        reprojected_slice, _ = reproject_interp(
            (slice, wcs_3d_1), wcs_target_slice, shape_out=target_shape_2d
        )
    ...
    fits.writeto(
        path_3Ddata_reprojected, reprojected_data_3d, header_reprojected, overwrite=True
    )
...
def get_skeleton_instances(skeletons):
    ...
    labeled_skeleton, num_labels = measure.label(
        skeletons, return_num=True, connectivity=2
    )
