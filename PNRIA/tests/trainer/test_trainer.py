...
@patch('PNRIA.dataset.BaseDataset', MockDataset)
...
def test_run_loop_validation(trainer_config, device):
    ...
    avg_loss = trainer.run_loop_validation(epoch=0, dataloader=dataloader)
...
def test_from_snapshot(tmp_path, trainer_config, device):
    ...
    assert str(next(trainer_loaded.model.parameters()).device) == str(device), "Loaded model is not on the expected device"
