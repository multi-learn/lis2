"""UNet_pe implementation.

Code from https://github.com/mateuszbuda/brain-segmentation-pytorch/blob/master/UNet_pe.py
"""
...
class UNet_pe(nn.Module):
    """
    UNet_pe_pe implementation with batch normalization.
    ...
    def __init__(self, in_channels=1, out_channels=1, init_features=64, encoding="sym"):
        super().__init__()
        ...
        self.pool1 = nn.MaxPool2d(
            kernel_size=UNet_pe._MAX_POOL_KERNEL_SIZE, stride=UNet_pe._MAX_POOL_STRIDE
        )
        ...
        self.upconv4 = nn.ConvTranspose2d(
            16 * features + 2, # +2 correspond to the 2 values of the position encoding
            features * 8,
            kernel_size=UNet_pe._UPCONV_KERNEL_SIZE,
            stride=UNet_pe._UPCONV_STRIDE,
        )
    ...
    def forward(self, x, position):
        ...
        bottleneck = torch.cat((bottleneck, position), dim=1)
                ...
                [
                    (
                        ...
                        nn.Conv2d(
                            ...
                            kernel_size=UNet_pe._CONV_KERNEL_SIZE,
                            padding=UNet_pe._CONV_PADDING,
                            bias=False,
                        ),
                    ),
    ...
    def position_encoding(self, positions):
        ...
        pe_x = torch.unsqueeze(torch.unsqueeze(pe_x.expand(pe_x.shape[0], 2), dim=2).expand(pe_x.shape[0], 2, 2), dim=1)
        pe_y = torch.unsqueeze(torch.unsqueeze(pe_y.expand(pe_y.shape[0], 2), dim=2).expand(pe_y.shape[0], 2, 2), dim=1)
