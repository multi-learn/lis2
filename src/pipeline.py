...
class KfoldsTrainingPipeline(Configurable):
    ...
    def parse_datasets_config(
        ...
        This method extracts and modifies dataset configurations for different stages of training,
        ensuring that each dataset (train, validation, and test) shares the same dataset type
        and dataset path as specified in the main configuration.

        Returns:
            Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
            A tuple containing the parsed configurations for:
                - The folds controller
    ...
    def run_inference(
        ...
        This method loads a trained model from the given snapshot and applies it to the
        test dataset, generating segmentation or classification results.

        Args:
            model_snapshot (str): Path to the trained model snapshot.
            test_config (Dict[str, Any]): Configuration for the test dataset, including
                dataset path and preprocessing details.

        Returns:
            Optional[Any]: Inference results generated by the model, or None if inference fails.
        """
        inference_config = {
